...
class UserModel extends DB
{
    ...
    public function ListItem1($user_id)
    {
        // Check if $user_id is a valid non-empty integer
        if (!is_numeric($user_id) || empty($user_id)) {
            // Handle the error, for example, you can return an empty result or throw an exception
            return false;
        }
        ...
        if ($stmt === false) {
            // Handle the error, for example, log the error or return an empty result
            return false;
        }
    ...
    {
        if (isset($user_id)  && isset($address) ) {
